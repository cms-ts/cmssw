...
void analyze_HI_TTreeReader_ZMM(const char * collision_type = "PbPb23", const char * sample_name = "data",
  ...
  // --- Load Data-Driven UE Swap Efficiency Fits (systFlag == 19) ---
  TF1* fit_swap_eff_pp = nullptr;
  std::map<std::pair<double, double>, TF1*> map_swap_eff_PbPb;
  ...
  // We only run this systematic on MC!
  if (systFlag == 19 && !isData) { 
      ...
      if (f_eff_data && !f_eff_data->IsZombie() && f_eff_mc && !f_eff_mc->IsZombie()) {
          if (!isPbPb) {
              fit_swap_eff_pp_data = (TF1*)f_eff_data->Get("fit_cen_0p0_2p5");
              fit_swap_eff_pp_mc = (TF1*)f_eff_mc->Get("fit_cen_0p0_2p5");
          } else {
              std::vector<std::pair<double, double>> cen_bins_pct = {
                  {0.0, 2.5}, {2.5, 5.0}, {5.0, 7.5}, {7.5, 10.0}, {10.0, 15.0}, {15.0, 20.0},{20.0, 30.0}
              };
              for (const auto& bp : cen_bins_pct) {
                  ...
                  if (f_eff_data->Get(func_name) && f_eff_mc->Get(func_name)) {
  ...
  if (isData) {
    if (collision_name.Contains("PbPb23")) {
     ...
     // Missing L2Residual
      ...
      Files.push_back("My24HIpp_L2Residual_AK2PF_new.txt");
      // 3. Absolute Response
  ...
  else if (collision_name.Contains("ppref24")) name_Uncertainty_file = "My24HIpp_AK2PF_NonClosure_missPUL3_new_Uncertainty.txt";
  ...
  if (!isData) {
    ...
    if (collision_name.Contains("PbPb23") || collision_name.Contains("PbPb24")) {
      ...
      std::string jer_sf_unc_file = "My24ppRef_JER_SFUncertainty_AK2PF_new.txt"; 
  ...
  while (fReader.Next()) {
    ...
    int ijetLeading = -1;
    int ijetLeading_signal = -1; // Track leading jet in the back-to-back signal window
...
/*
      // --- NEW: Apply Data/MC Swap Ratio Weight (systFlag 19) ---
      if (systFlag == 19 && !isData) {
          double eff_data = 1.0, eff_mc = 1.0;
          double pt_for_eval = jtpt_corr[ijetLeading];
          
          if (!isPbPb && fit_swap_eff_pp_data && fit_swap_eff_pp_mc) {
              eff_data = fit_swap_eff_pp_data->Eval(pt_for_eval);
              eff_mc   = fit_swap_eff_pp_mc->Eval(pt_for_eval);
          } else if (isPbPb) {
              double current_cen_pct = hiBin_to_use / 2.0;
              for (const auto& kv : map_swap_eff_PbPb_data) {
                  // Find the centrality bin this event belongs to
                  if (current_cen_pct >= kv.first.first && current_cen_pct < kv.first.second) {
                      eff_data = kv.second->Eval(pt_for_eval);
                      // Pull MC efficiency from the correspondingly keyed map
                      eff_mc   = map_swap_eff_PbPb_mc[kv.first]->Eval(pt_for_eval);
                      break;
                  }
              }
          }
          
          // Safety bounds: Prevent math errors and runaway small numbers
          if (eff_data < 0.05) eff_data = 0.05; 
          if (eff_data > 1.0)  eff_data = 1.0;
          if (eff_mc < 0.05)   eff_mc = 0.05;     
          if (eff_mc > 1.0)    eff_mc = 1.0;
          
          // The systematic weight: Data/MC ratio
          double weight_swap = eff_data / eff_mc;
          
          // --- NEW DEBUG: Catch Massive Weights ---
          // If the weight is larger than 1.5, MC efficiency is suspiciously low compared to Data
          if (weight_swap > 1.5) {
              std::cout << "\n[DEBUG SWAP] MASSIVE WEIGHT DETECTED!" << std::endl;
              std::cout << "  isPbPb: " << isPbPb << std::endl;
              std::cout << "  hiBin_to_use: " << hiBin_to_use << " (Centrality: " << (hiBin_to_use / 2.0) << "%)" << std::endl;
              std::cout << "  Jet pT (eval point): " << pt_for_eval << " GeV" << std::endl;
              std::cout << "  Evaluated Efficiency (Data): " << eff_data << std::endl;
              std::cout << "  Evaluated Efficiency (MC): " << eff_mc << std::endl;
              std::cout << "  Resulting Weight: " << weight_swap << std::endl;
          }
          // ----------------------------------------
          
          // Only fill the profile for jets in the signal window (if pointer exists)
          // (Uncomment the line below if p_swap_correction_vs_xZj is declared globally in your scope)
          // if (dPhi_Zj > 7 * pi_value / 8 && p_swap_correction_vs_xZj) p_swap_correction_vs_xZj->Fill(xZj, weight_swap, 1.0); 
          
          // Apply the correction to the global event scale!
          scale *= weight_swap; 
      }
      // -----------------------------------------------------------
      ...
      // --- Fill True MC Efficiency ---
      // If a valid Gen Z and Gen Jet exist back-to-back:
      if (!isData && ijetGenLeading_unfold != -1 && dPhi_Zj_Gen > 7 * pi_value / 8) {
          double gen_pt_val = genpt[ijetGenLeading_unfold];
          
          // --- NEW: Pure Swap Isolation ---
          // Denominator: Did the true leading jet survive the detector and pass pT > 30?
          if (iRecoMatchedToGenLeading != -1) {
              h2_swap_pure_den_vs_cen->Fill(hiBin_to_use, gen_pt_val, scale);
              
              // Numerator: Did it also win 1st place INSIDE THE SIGNAL WINDOW? (If no, it was swapped!)
              if (iRecoMatchedToGenLeading == ijetLeading_signal) {
                  h2_swap_pure_num_vs_cen->Fill(hiBin_to_use, gen_pt_val, scale);
              }
          }
          // --------------------------------
          
          // Denominator: All valid true jets
          h2_gen_pt_total_vs_cen->Fill(hiBin_to_use, gen_pt_val, scale);
          
          // Numerator: Did the reconstructed leading jet perfectly match this true jet?
          if (ijetGenLeading_unfold == iGenjetMatchedtoLeadingReco) {
              h2_gen_pt_matched_vs_cen->Fill(hiBin_to_use, gen_pt_val, scale);
          }
      }
      // ------------------------------------
...
double scale_reco = scale;
      ...
      if (systFlag == 19 && !isData) {
          ...
          scale_reco = scale * weight_swap; 
      }
      // -----------------------------------------------------------

      h_deltaPhi_Zj->Fill(dPhi_Zj, scale_reco);

      if (!isData && isLeadingJetMatched) {
        double dPhi_Zj_matched = RelativePhi(Z.Phi(), jtphi[ijetLeading]);
        h_deltaPhi_Zj_matched->Fill(dPhi_Zj_matched, scale_reco);
      }

      if (dPhi_Zj > 7 * pi_value / 8) {
        h_mumu_j->Fill(Z.M(), scale_reco);
        h_Z_pt_j->Fill(Z.Pt(), scale_reco);
        h_jet_pt_lj->Fill(jtpt_corr[ijetLeading], scale_reco);
        h_xZj->Fill(xZj, scale_reco);
        h_xZj_fixbinw->Fill(xZj, scale_reco);
        if (!isData && ijetGenLeading_unfold != -1 && dPhi_Zj_Gen > 7 * pi_value / 8) {
          h_response_unmatched->Fill(xZj, true_xZj, scale_reco);
          if (itotev < 0.6*Ngen) h_response_closure_unmatched->Fill(xZj, true_xZj, scale_reco);
        }
        if (itotev < 0.6*Ngen) h_xZj_train_closure->Fill(xZj, scale_reco);
        else h_xZj_test_closure->Fill(xZj, scale_reco);
        h_cen_j->Fill((hiBin_to_use)/2, scale_reco);
        h_HF_j->Fill(*hiHF, scale_reco);

        if (!isData && isLeadingJetMatched) {
          h_jet_pt_lj_matched->Fill(jtpt_corr[ijetLeading], scale_reco);
          h_xZj_matched->Fill(xZj, scale_reco);
        }
      }

      if (dPhi_Zj > 2 * pi_value / 3) h_jet_pt_lj_2pi_3->Fill(jtpt_corr[ijetLeading], scale_reco);
      h_jet_pt_lj_nocut->Fill(jtpt_corr[ijetLeading], scale_reco);
      ...
      if (RelativePhi(Z.Phi(), jtphi[ijetLeading]) > 7 * pi_value / 8) {
        if (!isData && ijetGenLeading_unfold != -1 && dPhi_Zj_Gen > 7 * pi_value / 8) {
          // Check if the leading gen jet is matched to leading reconstructed jet
          if (ijetGenLeading_unfold == iGenjetMatchedtoLeadingReco) {
            h_response->Fill(xZj, true_xZj, scale_reco); // Apply scaled weight here!
            h_xZj_reco->Fill(xZj, scale_reco);
            if (itotev < 0.6*Ngen) {
              h_response_closure->Fill(xZj, true_xZj, scale_reco);
              h_xZj_train_closure_matched->Fill(xZj, scale_reco);
            } else h_xZj_test_closure_matched->Fill(xZj, scale_reco);
          }
        }
      } // --- end filling information for unfolding ---

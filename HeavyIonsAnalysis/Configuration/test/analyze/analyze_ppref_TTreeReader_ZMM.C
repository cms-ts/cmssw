...
void analyze_ppref_TTreeReader_ZMM(bool isData = true) {
  ...
  TFile *file_output_ppref_mu;
    ...
    if (isData) {
      file_output_ppref_mu = new TFile("./output_ppref_mu_data.root", "RECREATE");
    }
    if (!isData) {
      file_output_ppref_mu = new TFile("./output_ppref_mu_MC.root", "RECREATE");
    }
...
//    iEvent++;
  ...
  c7->cd(1);
  h_deltaPhi_Zj->Draw();
  c7->Print("h_deltaPhi_Zj_ppref.pdf");

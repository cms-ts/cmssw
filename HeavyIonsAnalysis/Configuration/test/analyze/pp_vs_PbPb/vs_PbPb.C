...
//        std::string histo_name = "h_xZj";
        ...
        if (histo_name == "h_deltaPhi_Zj") {
          cout << "dphi<pi/3 after norm: " << 100*intgr_value_pi3_afternorm << " +-" << 100*intgr_error_pi3_afternorm << " %"
               << " ppref: " << 100*intgr_value_pi3_pp_afternorm << " +-" << 100*intgr_error_pi3_pp_afternorm << " %" << endl;
        }
        h->SetMarkerStyle(20);
        h->SetMarkerSize(1.);
        h->SetLineColor(1);

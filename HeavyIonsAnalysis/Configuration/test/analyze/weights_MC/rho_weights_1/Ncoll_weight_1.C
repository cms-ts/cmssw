...
void Ncoll_weight_1() {
        ...
        // Open MC file
        TFile* file_ = TFile::Open("./output_HI_mu_MC_rho_weights.root", "READ");
        TDirectoryFile* dir = (TDirectoryFile*)file_->Get("HI/Muons");
        TH1D* h = (TH1D*)dir->Get(histo_name.c_str());
        // Get data histogram
        TFile* file_data = TFile::Open("../Ncoll_weights_0/output_HI_mu_data_Ncoll_weights.root", "READ");
        ...
        // Print the canvas
        c->Print((histo_name + "_after.pdf").c_str());
}

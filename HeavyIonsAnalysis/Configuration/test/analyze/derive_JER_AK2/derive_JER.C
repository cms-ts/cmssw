...
JetSelect* js_PbPb = new JetSelect("../Winter24Prompt24_2024BCDEFGHI.root");

void derive_JER(std::string input_pattern, std::string output_name, int radius) {
    ...
    // DYNAMIC TREE NAME based on radius
    TString treeName = Form("akCs%dPFJetAnalyzer/t", radius);
...
/*    TTreeReaderValue<Int_t> nReco = {fReader, "nReco"};
    ...
    // DYNAMIC JEC FILE based on radius
    TString jecName = Form("../Prompt23HIPbPb_V1_MC_L2Relative_AK%dPF.txt", radius);
   ...
   /*         bool overlaps_with_muon = false;
            ...
            // DYNAMIC MATCHING CUT: R=0.2 -> 0.10, R=0.3 -> 0.15

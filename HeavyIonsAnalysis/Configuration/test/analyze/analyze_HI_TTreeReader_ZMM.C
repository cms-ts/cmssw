...
//Default isData, for MC root -l 'analyze_HI_TTreeReader_ZMM.C(false, 0)'
...
//      file_output_HI_mu = new TFile("./weights_MC/vz_weights_2/output_HI_mu_MC_rho_weights_after.root", "RECREATE");
    ...
    if (!isData) {
      file_output_HI_mu = new TFile("./weights_MC/final_weights_3/output_HI_mu_MC_final_weights.root", "RECREATE");
    }
  }

  else if (weight_phase == 4) {
    if (!isData) {
  ...
  while (fReader.Next()) {
    ...
    if (!isData) {
      ...
      if (weight_phase == 3 || weight_phase == 4) {
